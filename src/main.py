...
def load_training_plan_from_csv(csv_path: str) -> TrainingPlan:
	...
	if plan_name == "Krafttraining 2":
		...
		ex1_set2_reps = int(df.iloc[9, 1]) if pd.notna(df.iloc[9, 1]) else 0  # B10
		ex1_set2_weight = float(df.iloc[7, 1]) if pd.notna(df.iloc[7, 1]) else 0  # B8
		ex1_set3_reps = int(df.iloc[9, 1]) if pd.notna(df.iloc[9, 1]) else 0  # B11
		...
		ex2_dash_row = 17  # A18
		ex2_sets = df.iloc[ex2_kg_row:ex2_dash_row+1, 0].tolist().count('WD')
		ex2_set1_reps = int(df.iloc[13, 1]) if pd.notna(df.iloc[13, 1]) else 0  # B15
		ex2_set1_weight = float(df.iloc[12, 1]) if pd.notna(df.iloc[12, 1]) else 0  # B14
		ex2_set2_reps = int(df.iloc[14, 1]) if pd.notna(df.iloc[14, 1]) else 0  # B16
		ex2_set2_weight = float(df.iloc[12, 1]) if pd.notna(df.iloc[12, 1]) else 0  # B14
		ex2_set3_reps = int(df.iloc[14, 1]) if pd.notna(df.iloc[14, 1]) else 0  # B17
		...
		ex3_set3_reps = int(df.iloc[21, 1]) if pd.notna(df.iloc[21, 1]) else 0  # B23
		...
		ex4_set3_reps = int(df.iloc[25, 1]) if pd.notna(df.iloc[25, 1]) else 0  # B29
		ex4_set3_weight = float(df.iloc[22, 1]) if pd.notna(df.iloc[22, 1]) else 0  # B23
...
#define function input value 1. Trainingsplan Jan Persico.csv 
#return value of the function is instance of model data
#load data from 1.Trainingsplan CSV file
#create model instance of data name 'data1' 
#strcture for reading 1. Trainingsplan Jan Persico.csv :
#model data1 (string nameoftrainingplan = cell A2, model list exercise(exercise1(string name = cell A7, int sets = count "WD" between "KG" in cell A8 and "-" in cell A11, model set1(int reps = Cell B9, int weight = B8,model set2(int reps = Cell B10, int weight = B8))), exercise2(string name = cell A12, int sets = count "WD" between "KG" in cell A13 and "-" in cell A16, model list set(set1(int reps = Cell B14, int weight = B13),  model set2(int reps = Cell B15, int weight = B13)))))
#empty cells in row "KG" filled in model with value 0
...
#load data from 2.Trainingsplan CSV  file as instance of data name 'data2' 
#load data from 3.Trainingsplan CSV  file as instance of data name 'data3' 
#load data from 4.Trainingsplan CSV  file as instance of data name 'data4' 		
#load data from 5.Trainingsplan CSV  file as instance of data name 'data5' 
#load data from 6.Trainingsplan CSV  file as instance of data name 'data6' 
